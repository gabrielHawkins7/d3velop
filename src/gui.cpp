...
void GUI::begin(D3velop &d3velop) {
  ...
  std::vector<HelloImGui::DockableWindow> windows{imageWindow, devWindow,
                                                  logsWindow, controlsWindow};
...
void GUI::dev_view(D3velop &d3velop) {
  ...
  if (CollapsingHeader("OpenGL Config")) {
    ...
    Text("maxUniformBufferBindings: %i",
         d3velop.glconfig.maxUniformBufferBindings);
