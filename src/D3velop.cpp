...
void D3velop::newImage(PATH fn){
    ...
    for (const auto& e : exts) {
            if (fn.extension().string() == e) supported_ext =  true;
    }

    if(!supported_ext){
        UTIL::log(UTIL::ERROR,
            std::string("Unsupported Extension: ")
            .append(fn.extension().string()).c_str()
        );
        return;
    }
   ...
   try{
       ...
       UTIL::log(UTIL::ERROR, std::string("UNABLE TO OPEN FILE : ").append(e.what()).c_str());
       return;
   }
...
void D3velop::check_vips(){
    if(VIPS_INIT("D3VELOP")){
        ...
        UTIL::log(UTIL::ERROR, std::string("Unable to load vips...").append(vips_error_buffer_copy()).c_str());
        vips_error_exit(NULL);
    }else{
        app_state.vips_enabled = true;
        UTIL::log(UTIL::SUCCESS, std::string("VIPS ENABLED : ").append(vips_version_string()).c_str());
    }
}

void D3velop::get_glconfig(){
    D3GL::getGlConfigData(glconfig);
}
