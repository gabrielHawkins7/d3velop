...
const std::unordered_map<std::string,std::string> shader_includes = {
  {"BrightnessContrast_v3", "vec3 brightnessContrast( vec3 v, float b, float c ) { return ( v - 0.5 ) * c + 0.5 + b; }"},
  {"Exposure_v3","vec3 exposure(vec3 v, float a) { return v * pow(2., a); }"}
};
...
static std::string parse_includes(std::string glsl){
    ...
    while ((pos = glsl.find("#include", pos)) != std::string::npos) {
        ...
        std::string includeKey = glsl.substr(startQuote + 1, endQuote - startQuote - 1);
...
static SHADER_DATA getShaderData(std::filesystem::path path){
    ...
    if (vert_start != std::string::npos && vert_end != std::string::npos) {
        out_data.vert_data = data.substr(vert_start + 6, vert_end - (vert_start + 6));
    } else {
        out_data.vert_data = "";
    ...
    if (frag_start != std::string::npos && frag_end != std::string::npos) {
        out_data.frag_data = data.substr(frag_start + 6, frag_end - (frag_start + 6));
