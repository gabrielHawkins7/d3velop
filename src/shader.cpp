...
void Shader::createShader(){
	...
	GLint result;
	glGetShaderiv(program, GL_COMPILE_STATUS, &result);	// assigns result with compile operation status
	if (result == GL_FALSE)
	{
		int length;
		glGetShaderiv(program, GL_INFO_LOG_LENGTH, &length); // assigns length with length of information log
		char* infoLog = (char*)alloca(length * sizeof(char));	// allocate on stack frame of caller
		glGetShaderInfoLog(program, length, &length, infoLog);	// returns the information log for a shader object
