...
int D3GL::uploadTexture(VIPSBUFFERDATA vbd, TEXTURE_DATA tex){
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16, tex.width, tex.height, 0, GL_RGB, GL_UNSIGNED_SHORT, vbd.buf);
    return textureID;
};
...
void D3GL::getGlConfigData(GLCONFIG& gl_config){
    ...
    gl_config.shadingLangVersion = reinterpret_cast<const char*>(glGetString(GL_SHADING_LANGUAGE_VERSION));
    ...
    for (GLint i = 0; i < numExtensions; ++i) {
        gl_config.supportedExtensions.push_back(reinterpret_cast<const char*>(glGetStringi(GL_EXTENSIONS, i)));
    }
}
